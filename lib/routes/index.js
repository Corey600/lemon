 ...
 * 接口错误处理
 */
apiRouter.use(function* apiError(next) {
  ...
  try {
    ...
    if (config.has('debug') && config.get('debug')) {
      return yield (this.body = new RET(CODE.ERROR, err.stack))
    }
    return yield (this.body = new RET(CODE.ERROR, 'Internal Server Error'))
  }
})
 ...
 * 页面错误处理
 */
webRouter.use(function* webError(next) {
...
* use rest api routes
*/
apiRouter.use(blogRouter.api.routes(), blogRouter.api.allowedMethods())
apiRouter.use(consoleRouter.api.routes(), consoleRouter.api.allowedMethods())

/**
* use web pageroutes
*/
webRouter.use(blogRouter.web.routes(), blogRouter.web.allowedMethods())
webRouter.use(consoleRouter.web.routes(), consoleRouter.web.allowedMethods())
